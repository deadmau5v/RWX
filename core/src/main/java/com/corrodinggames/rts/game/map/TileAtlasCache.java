...
public class TileAtlasCache {
    ...
    public static boolean hasTransparencyInRect(Texture texture, Rect rect) {
        ...
        for (int i = iL; i < iL2; i++) {
            for (int i2 = iM; i2 < iM2; i2++) {
                if (Color.alpha(texture.a(i2, i)) != 255) {
                    return true;
                }
            }
        }
    ...
    public void blitWithPadding(int i, Texture texture, Rect rect) {
        ...
        this.paddingBlitPaint.setAntiAlias(z);
        this.paddingBlitPaint.setFilterBitmap(z);
        this.paddingBlitPaint.setDither(z);
    ...
    public static Rect getPaddingCornerRect(Rect rect, Rect rect2, int i, int i2, int i3) {
        if (i == 0) {
            ...
            rect2.bottom = rect.bottom + i2 + i3;
        } else {
            rect2.left = rect.left - i3;
            rect2.right = (rect.left - i2) - i3;
            rect2.top = rect.top - 0;
            rect2.bottom = rect.bottom + 0;
        }
        return rect2;
    ...
    public void computeAtlasSlotRect(int i, Rect rect) {
        ...
        rect.left = i2;
        rect.top = i3;
        rect.right = i2 + this.slotPixelWidth;
        rect.bottom = i3 + this.slotPixelHeight;
    }
}
