...
public abstract class PlayerTeam extends Serializable implements Comparable<PlayerTeam> {
    ...
    public void updateTeamConnectionStatus() {
        ...
        this.teamColorPaint.setColor(teamUnitCount);
        this.teamTextPaint.setColor(Color.argb(Color.alpha(teamUnitCount), (int) (Color.red(teamUnitCount) * 0.5f), (int) (Color.green(teamUnitCount) * 0.5f), (int) (Color.blue(teamUnitCount) * 0.5f)));
    }
    ...
    private static void d(String str) {
        ...
        for (int i = 0; i < 10; i++) {
            teamColorIds[i] = Color.parseColor(strArrSplit[i]);
        }
    }
                        ...
                        if (iB == 0) {
                            for (int i5 = 0; i5 < textureArr.length; i5++) {
                                if (textureArr[i5] != null) {
                                    textureArr[i5].a(i3, i2, Color.argb(iA2, (iArr2[i5] * iC) >> 8, (iArr3[i5] * iC) >> 8, (iArr4[i5] * iC) >> 8));
                                }
                            }
                        } else if (iC != iB) {
                            ...
                            for (int i6 = 0; i6 < textureArr.length; i6++) {
                                if (textureArr[i6] != null) {
                                    textureArr[i6].a(i3, i2, Color.argb(iA2, Utility.distance((int) (iB + (iArr2[i6] * f)), 0, 255), Utility.distance((int) (iB + (iArr3[i6] * f)), 0, 255), Utility.distance((int) (iB + (iArr4[i6] * f)), 0, 255)));
                                }
                            }
                        }
                    }
    ...
    public static void a(Texture texture, Texture[] textureArr, int[] iArr, int[] iArr2) {
        ...
        for (int i = 0; i < iArr.length; i++) {
            iArr3[i] = Color.red(iArr[i]);
            iArr4[i] = Color.green(iArr[i]);
            iArr5[i] = Color.blue(iArr[i]);
        }
        for (int i2 = 0; i2 < iL; i2++) {
            for (int i3 = 0; i3 < iM; i3++) {
                int iA = texture.a(i3, i2);
                int iA2 = Color.alpha(iA);
                if (iA2 == 0) {
                    if (Color.red(iA) > 0 || Color.green(iA) > 0 || Color.blue(iA) > 0) {
                        for (int i4 = 0; i4 < textureArr.length; i4++) {
                            if (textureArr[i4] != null) {
                                textureArr[i4].a(i3, i2, Color.argb(0, 0, 0, 0));
                            }
                        }
                    }
                } else {
                    int iC = Color.green(iA);
                    int iB = Color.red(iA);
                    int iD =Color.blue(iA);
                    ...
                    if (fMax > 15.0f) {
                        for (int i5 = 0; i5 < textureArr.length; i5++) {
                            if (textureArr[i5] != null) {
                                float f = fMax / 255.0f;
                                textureArr[i5].a(i3, i2, Color.argb(iA2, Utility.distance((int) (fMin + (iArr3[i5] * f)), 0, 255), Utility.distance((int) (fMin + (iArr4[i5] * f)), 0, 255), Utility.distance((int) (fMin + (iArr5[i5] * f)), 0, 255)));
                            }
                        }
                    }
                }
            ...
            for (int i3 = 0; i3 < iL; i3++) {
                ...
                if (iA2 > 0) {
                    ...
                    for (int i4 = 0; i4 < textureArr.length; i4++) {
                        ...
                        if (textureArr[i4] != null) {
                            textureArr[i4].a(i2, i3, Color.argb(iA2, iDistance, iDistance2, iDistance3));
                        }
                    }
                }
            }
