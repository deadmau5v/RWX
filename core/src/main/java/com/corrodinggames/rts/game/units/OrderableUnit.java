...
public abstract class OrderableUnit extends UnitBase {
    ...
    static {
        defaultTeamPaint.setARGB(128, 255, 255, 255);
        defaultTeamPaint.o();
        selectedTeamPaint = new GamePaint();
        selectedTeamPaint.set(defaultTeamPaint);
        selectedTeamPaint.setAntiAlias(true);
        selectedTeamPaint.setAntiAlias(true);
        selectedTeamPaint.setAntiAlias(true);
        selectedTeamPaint.o();
        aG = new PointF();
        ...
        aW = new PositionData();
        overlayFilterLightGreen = new PorterDuffColorFilter(Color.argb(255, 200, 255, 200), PorterDuff.Mode.MULTIPLY);
        overlayFilterGreen = new PorterDuffColorFilter(Color.argb(255, 70, 255, 70), PorterDuff.Mode.MULTIPLY);
        overlayFilterRed = new PorterDuffColorFilter(Color.argb(255, 255, 40, 40), PorterDuff.Mode.MULTIPLY);
        overlayFilterBlue = new PorterDuffColorFilter(Color.argb(255, 120, 120, 255), PorterDuff.Mode.MULTIPLY);
    ...
    private void updateTouchTargetCommand(float f, UnitCommand unitCommand, UnitStateTracker unitStateTracker) {
        ...
        if (unitCommand != null) {
            ...
            if (unitCommand.targetUnit.bI()) {
                ...
                float f2 = 21.0f;
                if (unitCommand.targetUnit.cc().isEmpty()) {
                    f2 = 11.0f;
                }
    ...
    public boolean c(float f) {
        ...
        float f6 = (rectFCF.left + rectFCF.right) * 0.5f;
        float f7 = (rectFCF.top + rectFCF.bottom) * 0.5f;
        graphicsEngine.k();
        drawShadow();
        if (fCD != 1.0f) {
    ...
    public PointF getShadowOffset() {
        tempPointF1.set(getShadowOffsetX(), getShadowOffsetY());
        return tempPointF1;
    }
    ...
    public boolean drawShadow() {
        if (this.shadowTexture != null && canDrawShadow()) {
            ...
            if (isUnitTransportWeight()) {
                Rect rectA_ = a_(true);
                RectF rectF = dB;
                rectF.set(f - this.eu, f2 - this.ev, f + this.eu, f2 + this.ev);
                graphicsEngine.k();
    ...
    public boolean isBuilding() {
        return GameEngine.getInstance().cameraFollowMode.intersect(getUnitMassBounds());
    }
    ...
    public PointF E(int i) {
        ...
        PointF pointFG = G(i);
        tempPointF2.set(pointFG.x + (Utility.fastCos(f) * fG), pointFG.y + (Utility.fastSin(f) * fG));
        return tempPointF2;
    }
    ...
    public PointF G(int i) {
        ...
        tempPointF3.set(fFastCos, fFastSin);
        return tempPointF3;
    }
    ...
    public void br() {
        ...
        rectF.set(cd());
        rectF.top *= gameEngine.tileMap.tileWorldSizeY;
        rectF.bottom *= gameEngine.tileMap.tileWorldSizeY;
        rectF.left *= gameEngine.tileMap.tileWorldSizeX;
        rectF.right *= gameEngine.tileMap.tileWorldSizeX;
        rectF.offset(this.posX, this.posY);
        rectF.offset(-getUnitAIState(), -getUnitAIPathfindStatus());
        rectF.top -= 10.0f;
        rectF.bottom += 10.0f;
        rectF.left -= 10.0f;
        rectF.right += 10.0f;
    ...
    public String getPlacementFailureReason(boolean z, PlayerTeam playerTeam) {
        ...
        if (!r().p()) {
            ...
            for (int i3 = i + rectCd.left; i3 <= i + rectCd.right; i3++) {
                for (int i4 = i2 + rectCd.top; i4 <= i2 + rectCd.bottom; i4++) {
    ...
    public void drawPlacementOverlay(int i) {
        ...
        if (!r().p()) {
            ...
            int i3 = pointA.y;
            r();
            int i4 = i2 + rectCd.left;
            int i5 = i3 + rectCd.top;
            int i6 = i2 + rectCd.right;
            int i7 = i3 + rectCd.bottom;
            if (i != -2) {
    ...
    PointF calculateMaxAttackRange(float f) {
        ...
        dE.set(this.posX + movementDeltaOffset.x, this.posY + movementDeltaOffset.y);
        return dE;
    }
