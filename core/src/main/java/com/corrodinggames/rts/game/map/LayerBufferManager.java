...
public final class LayerBufferManager {
    ...
    public void renderScorchMarksInCell(int i, int i2, GraphicsEngine graphicsEngine) {
        ...
        for (int i3 = 0; i3 < size; i3++) {
            ...
            if (gameObject instanceof ScorchMark) {
                ...
                if (worldBoundsRect.contains((int) scorchMark.posX, (int) scorchMark.posY)) {
    ...
    public void setRenderScale(float f) {
        ...
        if (f5 != 1.0f) {
            ...
            tileMap.tempRectTile.set(gameEngine.cameraBoundsEnabled);
            tileMap.tempRectTile.right = ((int) (tileMap.tempRectTile.left + (tileMap.tempRectTile.height() / f5))) + 2;
            tileMap.tempRectTile.bottom = ((int) (tileMap.tempRectTile.top + (tileMap.tempRectTile.height() / f5))) + 2;
        ...
        this.copyBlitPaint.setAntiAlias(z3);
        this.copyBlitPaint.setAntiAlias(z3);
        this.copyBlitPaint.setAntiAlias(z3);
        boolean z4 = false;
        for (int i3 = 0; i3 < this.gridCellsPerAxis; i3++) {
            try {
                for (int i4 = 0; i4 < this.gridCellsPerAxis; i4++) {
                    ...
                    layerBufferCell.screenDstRect.set(i5 + 1, i6 + 1, (i5 + this.cellBufferPixelSize) - 2, (i6 + this.cellBufferPixelSize) - 2);
                    if (layerBufferCell.screenDstRect.left <= f6 && layerBufferCell.screenDstRect.top <= f7) {
                        if (layerBufferCell.screenDstRect.right > f6) {
                            layerBufferCell.screenDstRect.right = (int) f6;
                        }
                        if (layerBufferCell.screenDstRect.bottom > f7) {
                            layerBufferCell.screenDstRect.bottom = (int) f7;
                        }
                        ...
                        if (layerBufferCell.screenDstRect.left < i7) {
                            layerBufferCell.screenDstRect.left = i7;
                        }
                        if (layerBufferCell.screenDstRect.top < i8) {
                            layerBufferCell.screenDstRect.top = i8;
                        }
                        if (layerBufferCell.screenDstRect.right > worldWidth) {
                            layerBufferCell.screenDstRect.right = worldWidth;
                        }
                        if (layerBufferCell.screenDstRect.bottom > worldHeight) {
                            layerBufferCell.screenDstRect.bottom = worldHeight;
                        }
                        if (!layerBufferCell.screenDstRect.isEmpty()) {
                            boolean z5 = false;
                            boolean z6 = true;
                            ...
                            layerBufferCell.tileSrcRect.set(layerBufferCell.screenDstRect);
                            layerBufferCell.tileSrcRect.offset(-i5, -i6);
                            layerBufferCell.screenDstRectF.set(layerBufferCell.screenDstRect);
                            layerBufferCell.screenDstRectF.offset(-f10, -f11);
                            layerBufferCell.screenDstRectF.offset(f8, f9);
                            if (layerBufferCell.fadeProgressRatio > 0.0f) {
                                layerBufferCell.fadeBlendPaint.setAntiAlias(z3);
                                layerBufferCell.fadeBlendPaint.setAlpha((int) ((1.0f - layerBufferCell.fadeProgressRatio) * 255.0f));
