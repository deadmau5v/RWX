...
public class Minimap {
    ...
    public void init(Context context) {
        ...
        this.paint4.setARGB(255, 255, 255, 255);
        ...
        for (int i = 0; i <= 10; i++) {
            ...
            this.pings[i].setColor(-16777216);
            this.pings[i].setStyle(Paint.Style.FILL);
            this.pings[i].c(i * 25);
        }
        this.lastPing = new GamePaint();
        this.lastPing.setColor(-16777216);
        this.lastPing.setStyle(Paint.Style.FILL);
        this.paint5.setARGB(255, 255, 0, 0);
        this.paint5.setStyle(Paint.Style.STROKE);
        this.paint5.setStrokeWidth(2.0f);
        this.paint6.setARGB(155, 255, 0, 0);
        this.paint6.setStyle(Paint.Style.STROKE);
        this.paint6.setStrokeWidth(2.0f);
        this.paint7.setARGB(200, 12, SlickToAndroidKeycodes.AndroidCodes.KEYCODE_11, 219);
        this.paint7.setStyle(Paint.Style.STROKE);
        this.paint7.setStrokeWidth(2.0f);
        this.paint8.setColor(-16711936);
        this.paint9.setColor(-256);
        this.paint10.setColor(-65536);
        this.paint11.setColor(darkenColor(this.paint8.getColor()));
        this.paint12.setColor(darkenColor(this.paint9.getColor()));
        this.paint13.setColor(darkenColor(this.paint10.getColor()));
        this.paint14.setARGB(210, 255, 255, 255);
    }
    ...
    public void setup() {
        ...
        for (Point point : gameEngine.tileMap.unitObjects) {
            this.list3.add(new StrategicPoint(this, point.x, point.y));
        }
    ...
    void drawFog(float f, float f2) {
        GamePaint gamePaint;
        GameEngine gameEngine = GameEngine.getInstance();
        this.rect4.set(0, (int) (f * this.height), (int) this.width, (int) (f2 * this.height));
        ...
        if (tileMap.fogEnabled) {
            ...
            gamePaint4.c(255);
            if (z) {
                gamePaint4.c((int) ((1.0f - ((1.0f - (gamePaint2.getAlpha() / 255.0f)) * (1.0f - (this.pings[7].getAlpha() / 255.0f)))) * 255.0f));
            }
                ...
                for (int i5 = i; i5 < i2; i5++) {
                    ...
                    while (i6 < i4) {
                        ...
                        if (b != 0) {
                            ...
                            i6 = i8;
                            rect.set(0 + ((int) (i7 * f3)), 0 + ((int) (i5 * f4)), 0 + ((int) ((i8 + 1) * f3)), 0 + ((int) ((i5 + 1) * f4)));
                            if (b == 10) {
        ...
        for (int i12 = -1; i12 < PlayerTeam.TEAM_NEUTRAL; i12++) {
            ...
            if (playerTeamK != null) {
                ...
                if (gameEngine.settingsEngine.useMinimapAllyColors) {
                    if (z) {
                        this.paint3.setColor(PlayerTeam.i(playerTeamK.teamColorId));
                        paint = this.paint3;
    ...
    public void updateElementPositions() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (!baseUnit.isDestroyed && baseUnit.unitTransportTarget == null && baseUnit.getWeight() && baseUnit.c_() && !baseUnit.u()) {
                ...
                baseUnit.unitCargoCount = pointWorldToScreen.x;
                baseUnit.unitCargoMax = pointWorldToScreen.y;
        ...
        for (int i2 = 0; i2 < size2; i2++) {
            ...
            if (!buildPreview.isBuilding && buildPreview.someFlagU) {
                ...
                buildPreview.gridX = pointWorldToScreen2.x;
                buildPreview.gridY = pointWorldToScreen2.y;
        ...
        for (StrategicPoint strategicPoint : this.list3) {
            ...
            if (gameEngine.tileMap.clampWorldX(playerTeam, strategicPoint.gridX, strategicPoint.gridY)) {
                ...
                strategicPoint.screenX = pointWorldToScreen3.x;
                strategicPoint.screenY = pointWorldToScreen3.y;
            }
        }
    }
    ...
    public void update(float f) {
        ...
        if (this.lastPingTime > 15.0f) {
            ...
            for (MinimapEffect minimapEffect2 : this.list2) {
                if (minimapEffect2.type != MinimapEffectType.message && gameEngine.cameraSmoothing.contains(minimapEffect2.x, minimapEffect2.y)) {
    ...
    public Point worldToScreen(float f, float f2) {
        ...
        this.point.set((int) ((f * this.scaleX * this.width) + this.x), (int) ((f2 * this.scaleY * this.height) + this.y));
        return this.point;
    }
    ...
    public Point screenToWorld(float f, float f2) {
        ...
        this.point.set((int) (((f - this.x) / this.width) * this.mapWidth), (int) (((f2 - this.y) / this.height) * this.mapHeight));
        return this.point;
    }
    ...
    public void draw(float f) {
        ...
        this.rect.set((int) this.x, (int) this.y, (int) (this.x + this.width), (int) (((double) (this.y + this.height)) - 0.4d));
        boolean z = false;
        boolean z2 = false;
        boolean z3 = false;
        ...
        if (!z && !z3) {
            this.paint2.setARGB(255, 100, 100, 100);
            this.paint2.setStrokeWidth(1.0f);
            if (GameUI.bO) {
                this.paint2.setARGB(115, 0, 0, 0);
            ...
            if (z3) {
                this.paint2.setARGB(255, 0, (int) (0.0f + (fFastSin * 230.0f)), 0);
            } else if (!z2) {
                this.paint2.setARGB(255, 12, (int) (0.0f + (fFastSin * 220.0f)), (int) (0.0f + (fFastSin * 220.0f)));
            } else {
                this.paint2.setARGB(255, (int) (0.0f + (fFastSin * 230.0f)), 0, 0);
            }
            this.paint2.setStrokeWidth(2.0f);
        }
        ...
        for (StrategicPoint strategicPoint : this.list3) {
            if (strategicPoint.isVisible) {
                this.rect3.set(strategicPoint.screenX, strategicPoint.screenY, strategicPoint.screenX + 2, strategicPoint.screenY + 2);
        ...
        if (this._graphicsOperations.size() != 0) {
            ...
            while (it.hasNext()) {
                ...
                if (minimapGraphicsOperation.unit.isDestroyed) {
                    ...
                    Point pointWorldToScreen = worldToScreen(baseUnit.posX, baseUnit.posY);
                    if (!baseUnit.a(pointWorldToScreen.x, pointWorldToScreen.y)) {
                        graphicsEngine.a(pointWorldToScreen.x, pointWorldToScreen.y, 4.0f, baseUnit.team.teamColorPaint);
                    }
                }
            }
        }
        ...
        for (int i4 = 0; i4 < i3; i4++) {
            ...
            if ((projectile.D || (projectile.q != null && projectile.q.D)) && projectile.j != null) {
                ...
                graphicsEngine.a(pointWorldToScreen2.x, pointWorldToScreen2.y, f3, projectile.j.team.teamColorPaint);
            }
        }
        Point pointWorldToScreen3 = worldToScreen(gameEngine.viewpointXSnapped, gameEngine.viewpointYSnapped);
        this.rect2.left = pointWorldToScreen3.x;
        this.rect2.top = pointWorldToScreen3.y;
        ...
        this.rect2.right = pointWorldToScreen4.x;
        this.rect2.bottom = pointWorldToScreen4.y;
        if (this.rect2.left < this.rect.left) {
            this.rect2.left = this.rect.left;
        }
        if (this.rect2.right > this.rect.right) {
            this.rect2.right = this.rect.right;
        }
        if (this.rect2.top < this.rect.top) {
            this.rect2.top = this.rect.top;
        }
        if (this.rect2.bottom > this.rect.bottom) {
            this.rect2.bottom = this.rect.bottom;
        }
        ...
        while (it2.hasNext()) {
            ...
            if (minimapEffect2.type == MinimapEffectType.unit) {
                ...
                paint.setARGB((int) (50.0f + (fFastSin2 * 190.0f)), (int) (50.0f + (fFastSin2 * 190.0f)), 0, 0);
            } else if (minimapEffect2.type == MinimapEffectType.message) {
                paint = this.paint7;
            } else {
                paint = this.paint5;
                float fFastSin3 = Utility.fastSin(this.field_r);
                paint.setARGB((int) (50.0f + (fFastSin3 * 190.0f)), (int) (50.0f + (fFastSin3 * 190.0f)), 0, 0);
            }
            float fClampTo255 = Utility.clampTo255(f4, f5, 1.0f);
            if (minimapEffect2.type == MinimapEffectType.unit) {
                float f6 = this.width * fClampTo255;
                float f7 = this.height * fClampTo255;
                graphicsEngine.a(clampX(pointWorldToScreen5.x - f6), clampY(pointWorldToScreen5.y - f7), clampX(pointWorldToScreen5.x + f6), clampY(pointWorldToScreen5.y + f7), paint);
                graphicsEngine.a(clampX(pointWorldToScreen5.x + f6), clampY(pointWorldToScreen5.y - f7), clampX(pointWorldToScreen5.x - f6), clampY(pointWorldToScreen5.y + f7), paint);
            } else {
                graphicsEngine.a(clampX(pointWorldToScreen5.x - (this.width * fClampTo255)), clampY(pointWorldToScreen5.y), clampX(pointWorldToScreen5.x + (this.width * fClampTo255)), clampY(pointWorldToScreen5.y), paint);
                graphicsEngine.a(clampX(pointWorldToScreen5.x), clampY(pointWorldToScreen5.y - (this.height * fClampTo255)), clampX(pointWorldToScreen5.x), clampY(pointWorldToScreen5.y + (this.height * fClampTo255)), paint);
            }
